...
def get_file_hash(path, algorithm='sha256', max_size=None, logger=None):
...
def collect_file_metadata(path, file_stat=None, logger=None):
    ...
    if stat.S_ISREG(file_stat.st_mode):
        if any(path.startswith(cpath) for cpath in HASH_CRITICAL_PATHS):
            file_hash = get_file_hash(path, logger=logger)
            if file_hash:
                metadata["sha256"] = file_hash

    return metadata

def file_changed(path, current_stat, prev_meta, logger=None):
    ...
    if any(path.startswith(p) for p in HASH_CRITICAL_PATHS):
        current_hash = get_file_hash(path, logger=logger)
